...
private:
    ...
    int m_poisonStatus;
    bool m_canMove;
    // TODO DONE: You'll probably find that a rabbit object needs additional
...
{
    return m_row;
    // DONE TODO: TRIVIAL:  Return what column the Rabbit is at
...
{
    return (m_poisonStatus > HAS_POISON);
    // DONE TODO: Return whether the Rabbit is dead
    // Delete the following line and replace it with the correct code.
}

void Rabbit::move()
{
    if (m_poisonStatus == HAS_POISON && !m_canMove)
    {
        m_canMove = true;
        return;
    }
    if (m_poisonStatus == EMPTY || m_canMove)
    {
        int dir = randInt(0, 3);
        ...
        if (m_canMove)
            m_canMove = false;
        if (m_poisonStatus == EMPTY && m_arena->getCellStatus(m_row, m_col) == HAS_POISON)
        {
            m_poisonStatus = HAS_POISON;
            m_canMove = false;
            m_arena->setCellStatus(m_row, m_col, EMPTY);
        }
        if (m_poisonStatus == HAS_POISON && m_arena->getCellStatus(m_row, m_col) == HAS_POISON)
        {
            m_poisonStatus++;
            m_arena->setCellStatus(m_row, m_col, EMPTY);
        }
...
Arena::~Arena()
{
    ...
    for (int i = 0; i < m_nRabbits; i++)
        delete m_rabbits[i];
    // DONE TODO:  Deallocate the player and all remaining dynamically allocated
    //        rabbits.
}
...
{
    return m_nRabbits;
    // DONE TODO: TRIVIAL:  Return the number of rabbits in the arena
    // Delete the following line and replace it with the correct code.
}
...
int Arena::numberOfRabbitsAt(int r, int c) const
{
    ...
    return numRabbitAt;
    // DONE TODO:  Return the number of rabbits at row r, column c
    // Delete the following line and replace it with the correct code.
}
...
void Arena::display(string msg) const
{
    ...
    for (r = 1; r <= rows(); r++)
        for (c = 1; c <= cols(); c++)
        {
            int numRabbitAt = numberOfRabbitsAt(r, c);
            if (numRabbitAt == 1)
                displayGrid[r - 1][c - 1] = 'R';
            else if (numRabbitAt >= 2 && numRabbitAt <= 8)
                displayGrid[r - 1][c - 1] = numRabbitAt + '0';
            else if (numRabbitAt >= 9)
                displayGrid[r - 1][c - 1] = '9';
        }
    // DONE TODO:  If one rabbit is at some grid point, set the displayGrid char
...
bool Arena::addRabbit(int r, int c)
{
    ...
    Rabbit *rabbit = new Rabbit(this, r, c);
    m_rabbits[m_nRabbits] = rabbit;
    m_nRabbits++;
    return true;
...
void Arena::moveRabbits()
{
    ...
    // Move all rabbits
    // DONE TODO:  Move each rabbit.  Mark the player as dead if necessary.
...
// calculates 3x3 grid of number of rabbits surrounding r, c
int calculateRabbitCounter(const Arena &a, int r, int c)
{
    int rabbitCounter = 0;
    int rStart = r - 1;
    int rEnd = r + 1;
    ...
    for (int j = rStart; j <= rEnd; j++)
    {
        ...
        {
            rabbitCounter += a.numberOfRabbitsAt(j, k);
        }
    }
    return rabbitCounter;
}

// Checks if player is surrounded by rabbits in the 4 directions
bool rabbitSurround(const Arena &a, int r, int c)
{
    int possibleDirs[NUMDIRS] = {-1, -1, -1, -1};
    int arrayIndex = 0;
    int origR = r;
    int origC = c;
    if (r - 1 > 0)
    {
        possibleDirs[arrayIndex] = NORTH;
        arrayIndex++;
    }
    if (r + 1 <= a.rows())
    {
        possibleDirs[arrayIndex] = SOUTH;
        arrayIndex++;
    }
    if (c - 1 > 0)
    {
        possibleDirs[arrayIndex] = WEST;
        arrayIndex++;
    }
    if (c + 1 <= a.cols())
    {
        possibleDirs[arrayIndex] = EAST;
        arrayIndex++;
    }
    int count;
    for (int i = 0; i < NUMDIRS; i++)
    {
        if (possibleDirs[i] >= 0)
        {
            attemptMove(a, possibleDirs[i], r, c);
...
int calculateRabbitArea(const Arena &a, int r, int c)
{
    int possibleDirs[NUMDIRS] = {-1, -1, -1, -1};
    int arrayIndex = 0;
    int rabbitCounter = 0;
    ...
    for (int i = 0; i < NUMDIRS; i++)
    {
        ...
        {
            attemptMove(a, possibleDirs[i], r, c);
            rabbitCounter += a.numberOfRabbitsAt(r, c);
            r = origR;
            c = origC;
        }
    }
...
bool recommendMove(const Arena &a, int r, int c, int &bestDir)
{
    ...
    int origR = r;
    int origC = c;
    // int rabbitMin = calculateRabbitCounter(a, r, c);
    int rabbitMin = calculateRabbitArea(a, r, c);
    bestDir = -1;
    ...
    for (int i = 0; i < NUMDIRS; i++)
    {
        if (attemptMove(a, i, r, c))
        {
            ...
            // int rabbitCounter = calculateRabbitCounter(a, r, c);
            // if (rabbitCounter < rabbitMin)
            // {
            //     rabbitMin = rabbitCounter;
            //     bestDir = i;
            // }
            int rabbitCounter = calculateRabbitArea(a, r, c);
            if (rabbitCounter < rabbitMin)
            {
                rabbitMin = rabbitCounter;
                bestDir = i;
